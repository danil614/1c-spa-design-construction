...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПриИзмененииСтатусаЗаказаНаЗавершен(Отказ);
КонецПроцедуры
...
Процедура ПриИзмененииСтатусаЗаказаНаЗавершен(Отказ)
	Если Отказ Тогда
		Возврат;
	КонецЕсли;

	Если СтатусЗаказа = Перечисления.СтатусЗаказа.Завершен Тогда
		// Проверяем, что все этапы завершены
		Для Каждого Этап Из СписокЭтапов Цикл
			Если Этап.СтатусЭтапа <> Перечисления.СтатусЭтапа.Завершен Тогда
				// Ошибка
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("Для завершения заказа необходимо завершить этап №%1", Этап.НомерСтроки);
				Сообщение.Поле = СтрШаблон("СписокЭтапов[%1].СтатусЭтапа", Этап.НомерСтроки - 1);
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.ПредыдущийСтатусЭтапа) Тогда
			...
			Если ТекущийСтатусЭтапа - ПредыдущийСтатусЭтапа = 1 Тогда
				...
				Если СтатусЗаказа = Перечисления.СтатусЗаказа.Согласование Тогда
