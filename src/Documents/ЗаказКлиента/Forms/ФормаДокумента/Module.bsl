...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Не Объект.Ссылка.Пустая() Тогда
		Если Не (Объект.СтатусЗаказа = Перечисления.СтатусЗаказа.Проект Или Объект.СтатусЗаказа
			= Перечисления.СтатусЗаказа.Согласование) Тогда

			// Делаем элементы формы только для просмотра
			Для Каждого Элемент Из Элементы Цикл
				Если ТипЗнч(Элемент) = Тип("ДекорацияФормы") Или ТипЗнч(Элемент) = Тип("ДополнениеЭлементаФормы") Тогда
					Продолжить;
				КонецЕсли;

				Если ТипЗнч(Элемент) = Тип("КнопкаФормы") Тогда
					// Оставляем доступными кнопки формы
					Если Не СтрНачинаетсяС(Элемент.Имя, "Форма") Тогда
			...
			// Оставляем доступными статусы
			Элементы.СтатусЗаказа.ТолькоПросмотр = Ложь;

			Элементы.Страницы.ТолькоПросмотр = Ложь;
			Элементы.ГруппаСписокЭтапов.ТолькоПросмотр = Ложь;
			Элементы.СписокЭтапов.ТолькоПросмотр = Ложь;
			Элементы.СписокЭтаповСтатусЭтапа.ТолькоПросмотр = Ложь;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
